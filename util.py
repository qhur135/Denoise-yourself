...
def get_input_exp(args, center=False) -> torch.Tensor:
    if args.noise_pc is not None and args.pc is not None:
        with open(args.noise_pc) as file:
            noise_pc = xyz2tensor(file.read(), force_normals=args.force_normal_estimation, k=args.k)
        with open(args.pc) as file:
            pc = xyz2tensor(file.read(), force_normals=args.force_normal_estimation, k=args.k)
    else:
        raise NotImplementedError('no recognized input type was found')
